...
def feature_calculator(args, graph):
    ...
    """
    idx_dict = {j:i for i, j in enumerate(graph.nodes)}

    index_1 = [idx_dict[edge[0]] for edge in graph.edges()]
    index_2 = [idx_dict[edge[1]] for edge in graph.edges()]

    values = [1 for edge in graph.edges()]

    # here>> TypeError: '>' not supported between instances of 'str' and 'int'
    # change value of node in copd_label_edges.txt to be int --> use dict {index: value } to convert
